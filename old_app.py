"""
Brain Tumor Analysis System (Classification + Planar Detection + Segmentation)
Integrates EfficientNet, ResNet18 (Planar), and DeepLabV3+ (Segmentation)
"""
...
class Config:
    ...
    # Map planar output (0,1,2) to model paths
    MODEL_SEGMENTATION = {
    ...
    # Mapping based on your training script: 0=Axial, 1=Coronal, 2=Sagittal
    PLANAR_CLASSES = ['axial', 'coronal', 'sagittal'] 
    
    IMG_SIZE_CLS = 224
    IMG_SIZE_SEG = 512  # DeepLab training size
    
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# ============================================================================
# 2. MODEL ARCHITECTURES (Embedded for Portability)
# ============================================================================

# --- A. Planar Classifier (ResNet18) ---
class PlaneClassifier(nn.Module):
    def __init__(self):
        super(PlaneClassifier, self).__init__()
        self.model = models.resnet18(weights=None) # Load structure only
        # Modify First Layer (3 channels -> 1 channel)
        self.model.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        # Modify Output Layer (1000 -> 3 classes)
...
# --- B. Segmentation Model (DeepLabV3+) ---
...
def load_models():
    ...
    if os.path.exists(Config.MODEL_TUMOR_CLASS):
        ...
        # Handle state dict key mismatch if saved differently
...
# Tumor Class (RGB, ImageNet stats)
...
# Planar & Seg (Grayscale, 0.5 stats)
...
def process_image(image_bytes, rotation_angle=0):
    """Run full pipeline: Classification -> Planar -> Segmentation"""
    ...
    if rotation_angle != 0:
        # expand=True changes the size of the image to fit the rotation
        # Note: PIL rotates counter-clockwise. To match standard clockwise UI expectations,
        # we often interpret positive input as clockwise or handle it in frontend.
        # Here we just blindly apply what the frontend sends.
        img_pil = img_pil.rotate(rotation_angle, expand=True)
    
    img_gray = img_pil.convert('L') # For Planar/Seg
    ...
    # --- Step 2: Planar Classification ---
    detected_plane = 'axial' # Default fallback
    if models_registry['planar']:
        ...
        with torch.no_grad():
            ...
            # Output is [Axial, Coronal, Sagittal] based on your training script
    ...
    if models_registry.get(seg_key):
        ...
        with torch.no_grad():
            ...
            # Visualization: Resize mask back to original display size (e.g., 512 for quality)
            ...
            overlay_arr[mask_arr > 127] = [255, 50, 50, 100] # Red with transparency
    ...
    # Add original (potentially rotated) image for preview
...
def predict():
    ...
    # Get rotation angle from form data (default 0)
